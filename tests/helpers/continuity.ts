...
export const continuityState = () => ({todos: [], permissionMode: "default" as const, collaborationMode: "build" as const, uiEvents: []});
export const continuityHost: ToolContextHost = {canUseTool: async () => ({behavior: "allow"}),
    getPermissionRules: () => ({allow: [], ask: [], deny: []}), getPermissionMode: () => "bypassPermissions",
