...
export function createTestMemoryRuntime(cwd: string, options: {enabled?: boolean; autoExtract?: boolean; consolidator?: MemoryConsolidator} = {}): MemoryRuntimeLike {
 return createMemoryRuntimeFactory({createConsolidator: () => options.consolidator ?? {async consolidate({lease, baseline}) {
