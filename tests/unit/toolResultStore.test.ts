...
describe("ToolResultStore", () => {
  ...
  test("不同 Session 不能通过相同 result id 互相读取", async () => {
    await withTempProject(async (cwd) => {
      const hicodeHome = join(cwd, "store");
      const first = createTestToolResultStore(cwd, "session-one", {hicodeHome});
      const second = createTestToolResultStore(cwd, "session-two", {hicodeHome});
