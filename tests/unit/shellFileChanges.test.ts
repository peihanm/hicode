...
test("Bash 不生成文件变化，UI、SDK 和恢复后仍保留失败命令", async () => {
    const start: AgentEvent = {type: "tool_call_start", turnId: "turn", toolCallId: "shell", name: "bash", args: "{\"command\":\"test\"}"};
    const end: AgentEvent = {type: "tool_call_end", turnId: "turn", toolCallId: "shell", outcome: "failed", result: "exit 1"};
    ...
    expect(store.getPersistedUIEvents().filter(event => event.type === "file_change")).toHaveLength(0);
    expect(sdk.getPersistedUIEvents().filter(event => event.type === "file_change")).toHaveLength(0);
    expect(events.filter(event => event.type === "item.completed" && event.item.type === "file_change")).toEqual([]);
    ...
    expect(restored.getSnapshot().threads.find(thread => thread.role === "file_change_group")).toBeUndefined();
});
