...
test("Root 使用标准工具单次接收 note，下一轮直接召回，不维护索引",async()=>withTempProject(async cwd=>{
 ...
 const result=await root("请记住保持简洁",createInitialHistory(cwd,"glm-test"),event=>{events.push(event.type);},createTestContext(cwd,{memoryFiles:memory.fileAccess(memoryOwner())}),EMPTY_AGENT_INPUT_CHANNEL,{getToolSchemas:tools.getToolSchemas,executeTool:tools.executeTool,isToolConcurrencySafe:tools.isConcurrencySafe});
 expect(result.reason).toBe("completed");expect(events).toContain("memory_update");expect((await memory.contextForTurn("继续")).block).toContain("保持简洁");expect((await memory.status()).pending).toBe(1);expect(tools.toolNames).not.toContain("memory");await memory.close();
}));
test("忽略 Memory 后即使模型试图读索引也被真实工具链拒绝",async()=>withTempProject(async cwd=>{
 ...
 await createMemoryAwareAgentRunner(createAgentRunner({callLLM:fake.callLLM,compactHistory:noCompact}),memory)("忽略记忆",createInitialHistory(cwd,"glm-test"),()=>{},createTestContext(cwd,{memoryFiles:memory.fileAccess(memoryOwner())}),EMPTY_AGENT_INPUT_CHANNEL,{getToolSchemas:tools.getToolSchemas,executeTool:tools.executeTool,isToolConcurrencySafe:tools.isConcurrencySafe});
