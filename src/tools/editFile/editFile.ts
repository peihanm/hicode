...
function readStateMessage(
    ...
    return `Use read_file to read ${path} before editing (to prevent stale writes)`;
}
...
async function validateEdits(
    ...
    for (const [editIndex, edit] of edits.entries()) {
        ...
        if (spans.length === 0) {
            return fail(`In ${path}, old_string was not found in the original version. Check the text and context; later edits cannot match content generated by earlier edits.`);
        }
