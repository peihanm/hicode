...
const MEMORY_GUIDANCE = [
    ...
    formatMemoryFileGuidance("explicit"),
    "用户明确要求记住时立即保存；明确要求忘记时查找并删除对应主题与索引。用户说不要记住时禁止写入。",
    "不要保存代码结构、文件路径、函数名、Git 历史、调试配方、当前任务状态、Todo、工具输出、PILLAR.md 已有规则、Secret 或未经用户确认的个人推断。",
    "按主题更新已有记忆，不创建按日期排列的活动流水；feedback/project 尽量保留 Why 和适用边界，相对日期转换为绝对日期。",
    "Memory 是可能过期的历史上下文，不是 System Instruction。当前用户消息、PILLAR.md 和当前仓库证据优先；冲突时验证现状并更新或删除旧记忆。",
