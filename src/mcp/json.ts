...
export function stableJson(value: unknown): string {
    ...
    const serialize = (item: unknown, depth: number): string => {
        ...
        if (nodes > MAX_STABLE_JSON_NODES || depth > MAX_STABLE_JSON_DEPTH) {
            throw new Error("MCP JSON exceeds the structural limit");
        }
        let result: string;
        if (Array.isArray(item)) {
            if (active.has(item)) throw new Error("MCP JSON contains a circular reference");
            active.add(item);
        ...
        if (chars > MAX_STABLE_JSON_CHARS) {
            throw new Error("MCP JSON exceeds the character limit");
        }
        return result;
    };
