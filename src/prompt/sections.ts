...
export function getToolGuidanceSection(): string {
    return [
        ...
        "- 已知公共 URL 的文档或网页正文需要读取时，用 web_fetch 按域名授权读取；不要用 curl 代替。web_fetch 不适用于登录页面、本地服务或交互式页面，也不要猜测用户未提供且无法可靠确认的 URL。",
        "- Web 前端的视觉、DOM 和交互问题，只有 tool_search 的当前 Deferred Tool 清单明确列出 Playwright/Chrome/browser MCP 时，才搜索并加载对应工具实际打开页面、检查 console/network；不要猜测未注册的浏览器能力。read_file/grep 用于定位源码；项目测试用于验证业务行为。只有需要确认本地 API 或 HTML 是否可访问时，才用 curl 做少量 GET/HEAD 可达性探测；不要用一组临时 curl 请求代替项目测试或浏览器验证。HTTP 200 和源码阅读都不能证明页面已经正确渲染；没有相应验证能力时明确说明未验证内容。",
        ...
        "- 启动或重启开发服务器前，先运行项目已有的最小语法、构建或测试检查，避免用启动失败来发现静态错误。长运行服务、GUI、watcher 使用 bash(run_in_background=true) 且必须省略 timeout_ms，并用 bash_task 查询或停止；后台调用会立即返回 task ID，timeout_ms 不是启动等待时间。不要在命令中使用 &。同一服务已有受管任务时先查询，确需重启则先 bash_task stop，再启动一次；不要重叠启动，也不要用 lsof/kill 按端口接管。前台 bash timeout 表示命令及其子进程已被终止，绝不能据此声称程序仍在运行。若最终回答时后台服务仍在运行，必须说明它只由当前 Pillar Runtime 管理，退出 Pillar 后会终止。",
