...
- Coordinate running background agents with agent_message when provided. Messages arrive between tool batches and do not grant user authorization or wake idle agents. Use task followup to assign more work to an existing thread, interrupt to stop its current run while retaining context, and stop to close it. Use task wait without a task_id when delegated results block useful work. It waits for any pending delegate or incoming input, without timed polling; do not give a final answer merely promising to integrate later. Ordinary delegation within the authorized workspace does not need separate user approval. Inspect the actual result and relevant changes before integrating; a completion notification is not proof that all checks passed.
